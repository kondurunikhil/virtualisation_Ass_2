...
switch(eax) {
		case 0x4FFFFFFC:
			...
			printk(KERN_INFO " Total exits count EAX = %u", eax);
			break;

		case 0x4FFFFFFD:
		ebx = (atomic64_read(&total_cup_cycles_counter) >> 32);;				 ecx = (atomic64_read(&total_cup_cycles_counter) & 0xFFFFFFFF);		
		printk(KERN_INFO "(hi) Total Exit CPU  Cycle Time in ebx  = %u", ebx);
		printk(KERN_INFO "(lo) Total Exit CPU Cycle Time(lo) in ecx = %u", ecx);
		break; 
		default:
